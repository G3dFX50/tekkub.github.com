...
<table id="addon_list">
  {% for addon in page.addons %}
    <tr>
      ...
      <td> "Bugs":http://code.google.com/p/tekkub-wow/issues/list?can=1&q=addon:{{ addon.name }}&sort=status+milestone&colspec=ID%20Type%20Status%20Addon%20Milestone%20Summary </td>
      <td> "Repo":http://github.com/tekkub/{{ addon.name | downcase }} </td>
    </tr>
  {% endfor %}
</table>
